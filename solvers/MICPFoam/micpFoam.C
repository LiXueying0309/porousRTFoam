...
int main(int argc, char *argv[])
{
    ...
    while (runTime.loop())
    {
        ...
        volScalarField  ut ("ut",fvc::ddt(Curea));       
        {
            #include "UEqn.H"
            ...
            Curea=Curea - b*MbbyMmax*Mmax*Ku*(Curea/(Km+Curea))*(KNH4/(KNH4+CNH4))*eps*mesh.time().deltaTValue(); 
            mt=fvc::ddt(Ccaco3);
            ut=fvc::ddt(Curea);
            eps= eps - a*(1./rhoCaCO3)*mt*eps.oldTime()+a*(1./rhoUrea)*ut*MbbyMmax*eps.oldTime(); 
